...
function currencyRequest(callback) {
    ...
    // var queryParam = {};
    var APIKEY = process.env.CURRENCY_API_KEY;  // Make sure an environment variable is set, containing a valid APOD key
    //declare and initialize. pulling the API key value from the environment variable//
    ...
    request( {uri :baseURL, qs: queryParam} , function(error, currency_response, body){
        ...
        if (!error && currency_response.statusCode == 200){
            ...
            var jsonForTemplate = currencyJSON.rates;  //grabbing "rates" from JSON returned.
...
// function processCURRENCYresponse(currencyJSON){
//
